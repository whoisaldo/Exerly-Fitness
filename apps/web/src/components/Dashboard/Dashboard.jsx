...
function AnimatedNumber({ value }) {
  const num =
    typeof value === 'number' ? value : parseInt(String(value).replace(/[^0-9-]/g, ''), 10) || 0;
...
export default function Dashboard() {
        ...
        if (dashDataRes.ok) {
          ...
          if (goalsData.weekly_workouts) {
            progress.weeklyWorkouts = {
              ...
              percentage: Math.min(
                100,
                ((dashData.workoutCount || 0) / goalsData.weekly_workouts) * 100
              ),
            };
          }
          ...
          if (goalsData.daily_calories) {
            progress.dailyCalories = {
              ...
              percentage: Math.min(
                100,
                ((dashData.totalConsumed || 0) / goalsData.daily_calories) * 100
              ),
            };
          }
          ...
          if (goalsData.sleep_hours) {
            progress.sleepHours = {
              ...
              percentage: Math.min(
                100,
                ((dashData.totalSleepHours || 0) / goalsData.sleep_hours) * 100
              ),
            };
          }
  ...
  const consumed =
    parseInt(
      String(stats.find((s) => s.label === 'Calories Consumed')?.value).replace(/[^0-9]/g, ''),
      10
    ) || 0;
  const burned =
    parseInt(
      String(stats.find((s) => s.label === 'Calories Burned')?.value).replace(/[^0-9]/g, ''),
      10
    ) || 0;
  const workouts =
    parseInt(
      String(stats.find((s) => s.label === 'Total Workouts')?.value).replace(/[^0-9]/g, ''),
      10
    ) || 0;
    ...
    <PageTransition className="min-h-screen bg-deep px-4 pb-24 pt-6 sm:px-6 lg:px-8">
      ...
      <AnimatePresence>
        {error && (
          ...
          >
            ...
            <button
              onClick={() => setError('')}
              className="shrink-0 rounded-lg p-1 hover:bg-error/20"
            >
              <svg
                className="h-4 w-4"
                fill="none"
      ...
      {!loading && (
        ...
        >
          <GlassCard
            elevated
            className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-around"
            hover={false}
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-slate-50">
                  {consumedAnim} <span className="text-sm font-normal text-slate-400">cal</span>
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xl font-bold text-slate-50">
                  {burnedAnim} <span className="text-sm font-normal text-slate-400">cal</span>
                </p>
              </div>
            </div>
        ...
        >
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            ...
            {goalsProgress.sleepHours && (
              <GlassCard className="flex flex-col items-center gap-3 text-center">
                <ProgressRing value={goalsProgress.sleepHours.percentage} size={88} label="Sleep" />
                <p className="text-sm text-slate-300">
                  <span className="font-semibold text-slate-50">
            ...
            <div className="flex flex-col items-center justify-center py-10 text-center">
              ...
              <div className="mt-5 flex flex-wrap justify-center gap-2">
                <Link to="/dashboard/activities">
                  <ActionButton variant="secondary" className="h-11">
                    Log Workout
                  </ActionButton>
                </Link>
      ...
      >
        <div className="relative overflow-hidden rounded-2xl border border-violet-500/20 bg-gradient-to-br from-violet-600/10 via-fuchsia-600/5 to-transparent p-6 shadow-glow-primary sm:p-8">
          ...
          <div className="relative flex flex-col items-start gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <h3 className="text-lg font-bold text-slate-50">AI-Powered Coaching</h3>
